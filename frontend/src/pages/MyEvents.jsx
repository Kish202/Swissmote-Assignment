...
const MyEvents = () => {
  ...
  const handleEventdelete = (eventId) => {
    api
      .delete(`/api/events/my-events/${eventId}/delete-event`)
      .then((response) => {
        ...
        <div className="grid grid-cols-1 max-h-[30rem] overflow-y-auto scrollbar-hide gap-6 ">
          {loading ? (
            ...
            events.map((event) => (
              <Card
                key={event._id}
                className="flex items-center px-4 py-2 gap-6 shadow-2xl "
              >
                <img
                  src={event.image}
                  alt={event.eventName}
                ...
                <CardContent className="flex-1 py-4">
                  ...
                  <div className="text-sm bg-gray-100 px-2 py-1 rounded w-fit mt-1">
                    {event.category}
                  </div>
